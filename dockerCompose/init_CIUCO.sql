...
create table app.levels (
	...
	name varchar(100) not null unique,
	description varchar (140),
	organization integer default 1 references app.organizations on delete set default not null
);
...
create table app.contents (
   ...
   activity_type_version integer default 1 references app.activity_type_version on delete set default not null,-- it doesnt get deleted, the version just changes its status to DELETED
   model jsonb not null,
